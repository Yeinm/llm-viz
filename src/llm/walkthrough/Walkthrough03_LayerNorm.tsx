...
上一节中的 ${c_blockRef('_输入嵌入_', state.layout.residual0)} 矩阵是我们第一个变换器（Transformer）块的输入。

变换器块的第一步是对该矩阵应用 _层归一化（LayerNorm）_。这是一个
对矩阵中每一列的值分别进行归一化的操作。`;
    breakAfter();
...
我们可以单独考察每一列，所以现在先聚焦于第 4 列（${c_dimRef('t = 3', DimStyle.T)}）。`;
    ...
    breakAfter();
    commentary(wt)`
目标是让这一列的平均值等于 0、标准差等于 1。为此，
我们需要求出这一列的这两个量（${c_blockRef('均值（mean，\u03bc）', ln.lnAgg1)} & ${c_blockRef('标准差（std dev，\u03c3）', ln.lnAgg2)}），然后减去平均值并除以标准差。`;
    ...
    breakAfter();
    commentary(wt)`
这里我们使用的记号是：用 E[x] 表示平均值，用 Var[x] 表示方差（针对长度为 ${c_dimRef('C', DimStyle.C)} 的列）。方差
其实就是标准差的平方。引入 epsilon 项（ε = ${<>1&times;10<sup>-5</sup></>}）是为了防止除以零。

由于我们要把这些值应用到该列的所有数值上，因此我们会在聚合层中计算并存储它们。

最后，一旦我们得到归一化后的值，就会让这一列中的每个元素乘以一个经过学习的
${c_blockRef('权重（weight，\u03b3）', ln.lnSigma)}，再加上一个 ${c_blockRef('偏置（bias，β）', ln.lnMu)}，最终得到我们的 ${c_blockRef('归一化值', ln.lnResid)}。`;
    ...
    breakAfter();
    commentary(wt)`
我们对 ${c_blockRef('输入嵌入矩阵', layout.residual0)} 的每一列都执行这一归一化操作，得到的结果
就是 ${c_blockRef('归一化输入嵌入', ln.lnResid)}，它已经准备好被送入自注意力（Self-Attention）层。
`;
