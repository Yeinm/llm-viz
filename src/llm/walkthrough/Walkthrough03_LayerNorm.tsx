...
${L('The ', '上一节中的 ')}${c_blockRef(L('_input embedding_', '_输入嵌入_'), state.layout.residual0)}${L(' matrix from the previous section is the input to our first Transformer block.', ' 矩阵是我们第一个变换器（Transformer）块的输入。')}

${L('The first step in the Transformer block is to apply _layer normalization_ to this matrix. This is an', '变换器块的第一步是对该矩阵应用 _层归一化（LayerNorm）_。这是一个')}
${L('operation that normalizes the values in each column of the matrix separately.', '对矩阵中每一列的值分别进行归一化的操作。')}`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
${L('Normalization is an important step in the training of deep neural networks, and it helps improve the', '归一化是深度神经网络训练中的一个重要步骤，它有助于提高')}
${L('stability of the model during training.', '模型在训练过程中的稳定性。')}

${L('We can regard each column separately, so let\'s focus on the 4th column (', '我们可以单独考察每一列，所以现在先聚焦于第 4 列（')}${c_dimRef('t = 3', DimStyle.T)}${L(') for now.', '）。')}`;
    ...
    breakAfter();
    commentary(wt)`
${L('The goal is to make the average value in the column equal to 0 and the standard deviation equal to 1. To do this,', '目标是让这一列的平均值等于 0、标准差等于 1。为此，')}
${L('we find both of these quantities (', '我们需要求出这一列的这两个量（')}${c_blockRef(L('mean (\u03bc)', '均值（mean，\u03bc）'), ln.lnAgg1)}${L(' & ', ' & ')}${c_blockRef(L('std dev (\u03c3)', '标准差（std dev，\u03c3）'), ln.lnAgg2)}${L(') for the column and then subtract the average and divide by the standard deviation.', '），然后减去平均值并除以标准差。')}`;
    ...
    breakAfter();
    commentary(wt)`
${L('The notation we use here is E[x] for the average and Var[x] for the variance (of the column of length ', '这里我们使用的记号是：用 E[x] 表示平均值，用 Var[x] 表示方差（针对长度为 ')}${c_dimRef('C', DimStyle.C)}${L('). The', ' 的列）。方差')}
${L('variance is simply the standard deviation squared. The epsilon term (ε = ', '其实就是标准差的平方。引入 epsilon 项（ε = ')}${<>1&times;10<sup>-5</sup></>}${L(') is there to prevent division by zero.', '）是为了防止除以零。')}

${L('We compute and store these values in our aggregation layer since we\'re applying them to all values in the column.', '由于我们要把这些值应用到该列的所有数值上，因此我们会在聚合层中计算并存储它们。')}

${L('Finally, once we have the normalized values, we multiply each element in the column by a learned', '最后，一旦我们得到归一化后的值，就会让这一列中的每个元素乘以一个经过学习的')}
${c_blockRef(L('weight (\u03b3)', '权重（weight，\u03b3）'), ln.lnSigma)}${L(' and then add a ', '，再加上一个 ')}${c_blockRef(L('bias (β)', '偏置（bias，β）'), ln.lnMu)}${L(' value, resulting in our ', '，最终得到我们的 ')}${c_blockRef(L('normalized values', '归一化值'), ln.lnResid)}${L('.', '。')}`;
    ...
    breakAfter();
    commentary(wt)`
${L('We run this normalization operation on each column of the ', '我们对 ')}${c_blockRef(L('input embedding matrix', '输入嵌入矩阵'), layout.residual0)}${L(', and the result is', ' 的每一列都执行这一归一化操作，得到的结果')}
${L('the ', '就是 ')}${c_blockRef(L('normalized input embedding', '归一化输入嵌入'), ln.lnResid)}${L(', which is ready to be passed into the Self-Attention layer.', '，它已经准备好被送入自注意力（Self-Attention）层。')}
`;
