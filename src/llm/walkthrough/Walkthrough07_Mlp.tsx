...
${L('The next half of the transformer block, after the self-attention, is the MLP (multi-layer perceptron). A bit of a mouthful, but here it\'s a simple neural network with two layers.', 'Transformer 块中紧随自注意力之后的后半部分，就是 MLP（多层感知机，multi-layer perceptron）。名字有点拗口，不过在这里它就是一个只有两层的简单神经网络。')}

${L('Like with self-attention, we perform a ', '与自注意力类似，在向量进入 MLP 之前，我们同样会先执行一次 ')}${c_blockRef(L('layer normalization', '层归一化（Layer Normalization）'), block.ln2.lnResid)}${L(' before the vectors enter the MLP.', '。')}

${L('In the MLP, we put each of our ', '在 MLP 中，我们把每个长度为 ')}${c_dimRef('C = 48', DimStyle.C)}${L(' length column vectors (independently) through:', ' 的列向量（彼此独立地）依次经过：')}

${L('1. A ', '1. 一次 ')}${c_blockRef(L('linear transformation', '线性变换'), block.mlpFcWeight)}${L(' with a ', '，并加上 ')}${c_blockRef(L('bias', '偏置'), block.mlpFcBias)}${L(' added, to a vector of length ', '，得到一个长度为 ')}${c_dimRef('4 * C', DimStyle.C4)}${L('.', ' 的向量。')}

${L('2. A GELU activation function (element-wise)', '2. 一个 GELU 激活函数（逐元素）')}

${L('3. A ', '3. 一次 ')}${c_blockRef(L('linear transformation', '线性变换'), block.mlpProjWeight)}${L(' with a ', '，并加上 ')}${c_blockRef(L('bias', '偏置'), block.mlpProjBias)}${L(' added, back to a vector of length ', '，再变回长度为 ')}${c_dimRef('C', DimStyle.C)}

${L('Let\'s track one of those vectors:', '让我们来跟踪其中一个向量：')}
`;
    breakAfter();
...
commentary(wt)`
${L('We first run through the matrix-vector multiplication with bias added, expanding the vector to length ', '我们首先执行带偏置的矩阵-向量乘法，把向量扩展到长度为 ')}${c_dimRef('4 * C', DimStyle.C4)}${L('. (Note that the output matrix is transposed here. This is purely for vizualization purposes.)', '。（请注意，这里的输出矩阵被转置了。这纯粹是为了便于可视化展示。）')}
`;
    breakAfter();
...
commentary(wt)`
${L('Next, we apply the GELU activation function to each element of the vector. This is a key part of any neural network, where we introduce some non-linearity into the model. The specific function used, GELU, looks a lot like a ReLU function (computed as ', '接下来，我们对向量的每个元素应用 GELU 激活函数。这是任何神经网络中的关键部分，我们由此为模型引入一些非线性。这里所用的具体函数 GELU，看起来与 ReLU 函数（计算方式为 ')}${<code>max(0, x)</code>}${L('), but it has a smooth curve rather than a sharp corner.', '）非常相似，但它的曲线是平滑的，而不是锐利的折角。')}
...
commentary(wt)`
${L('We then project the vector back down to length ', '然后，我们再通过一次带偏置的矩阵-向量乘法，把向量投影回长度为 ')}${c_dimRef('C', DimStyle.C)}${L(' with another matrix-vector multiplication with bias added.', '。')}
`;
    breakAfter();
...
commentary(wt)`
${L('Like in the self-attention + projection section, we add the result of the MLP to its input, element-wise.', '与「自注意力 + 投影」部分类似，我们把 MLP 的结果逐元素地加到它的输入上。')}
`;
    breakAfter();
    ...
    breakAfter();
commentary(wt)`
${L('We can now repeat this process for all of the columns in the input.', '现在，我们可以对输入中的所有列重复这个过程。')}`;
...
commentary(wt)`
${L('And that\'s the MLP completed. We now have the output of the transformer block, which is ready to be passed to the next block.', '到这里，MLP 就完成了。我们现在得到了 Transformer 块的输出，它已经准备好传给下一个块了。')}
`;
